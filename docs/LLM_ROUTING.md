...
### 3.1 LLM을 쓰는 기능 목록 (2026-09-25 정리)

`LlmPort`를 거치는 목적 11개와, 목적 enum 밖에서 LLM을 쓰는 것 2개. 이 표 밖에서는 LLM을 부르지 않는다(가드레일·주문·손익·시세 계산은 순수 코드).

admin 화면(F22)에서는 이 목적들을 **카테고리 다섯 개**로 묶어 고른다: **토론**(1·2·3·10) · **리포트·요약**(5·8·9) · **거래 판단**(6·7) · **추천**(4) · **번역**(11). 12는 로컬 고정, 13은 별도 프로세스. 카테고리 ↔ 목적 표는 `engine.llm.LlmCategory` 한곳.

| # | 기능 | 목적(`LlmPurpose`) | 호출 시점·빈도 | 개인 데이터 |
|---|---|---|---|---|
| 1 | F6 토론 — 페르소나 발언 | `PERSONA_TURN` | 라운드 × 인원, 사용자 요청 시 | 없음(매도 토론은 보유 수량 포함 가능 → `PERSONAL`) |
| 2 | F6 토론 — 사회자(종합·결론·반대 논거·등급 강제) | `DEBATE_MODERATOR` | 라운드 끝·결론 요청 시 | 없음 |
| 3 | 자유 서술 → `Verdict`·추천 레코드 스키마 변환 | `STRUCTURED_EXTRACTION` | 결론·추천마다 | 없음 |
| 4 | F5 추천 2단 후보 분석·약식 토론 | `RECOMMENDATION_ANALYSIS` | 스크리닝 뒤 수십 종목, 일 1회 | 없음(F13 국민연금 자료 포함) |
| 5 | F9 일간·주간 시장 리포트 | `MARKET_REPORT` | 장 전 일 1회 + 주 1회 | 보유·관심 절 포함 시 `PERSONAL` |
| 6 | F8 자동 매도 판단(당일 매도가 이득인가) | `SELL_DECISION` | 허용 종목마다 장중 | **있음**(단가·수량) → 실패 시 판단 보류 |
| 7 | F7(b) 급등 재료 확인("실체 있는 재료가 있는가") | `SURGE_CATALYST_CHECK` | 탐지기 신호마다, 3~5초 | 없음 → 실패 시 매수 안 함 |
| 8 | 요약·분류: 개요·전망 요약(F6, 주 1회 캐시), 업종 요약(F15), ETF 요약(F15, 보류), 공시 분류·악재 유형 판정 보조(DART·EDGAR), 여론 요약(F12, 휴면) | `DOCUMENT_DIGEST` | 배치·최초 열람 | 없음 |
| 9 | F10 회고 작성 | `RETROSPECTIVE` | 크게 틀리거나 맞은 사례마다 | **있음**(매매 결과) |
| 10 | 종목 질의응답·토론 개입 응답 | `USER_CHAT` | 사용자 입력 시, 스트리밍 | 상황에 따라 |
| 11 | F12 영→한 번역(StockTwits 휴면, Massive 뉴스 재사용 검토) | `TRANSLATE` | 목록 배치·펼칠 때 | 없음 |
| 12 | RAG 임베딩(`bge-m3`, 로컬 Ollama) | `EmbeddingPort`(별도 포트) | 문서 색인·질의마다 | 원문(로컬 처리) |
| 13 | 미러피시 심층 토론 | `SimulationPort`(자체 LLM 키, 별도 프로세스) | 심층 토론 요청 시 | 시드 문서에 여론·수급 요약 |

- LLM 출력은 어느 경우에도 주문 파라미터가 되지 않는다(구조화 → 스키마 검증 → 가드레일 → 승인). 6·7은 "판단"만 내고 실행 여부는 자동화 규칙이 정한다.
- 가치사슬 후보 추출(F15 관련 종목, 사업보고서에서 LLM이 제안)은 후순위이며 채택 시 `DOCUMENT_DIGEST`로 묶는다.
- 개인 데이터 열이 "있음"인 기능은 사용자별 "개인 데이터 LLM 전송 허용"(8장, F21 회원정보 › 보안)이 꺼진 제공자로 가지 않는다.
...
> 경로를 편집하는 화면(admin 전용, 등록 제공자만 선택, 프리셋, 검증, 이력)은 [LLM_ROUTE_SETTINGS_DESIGN.md](LLM_ROUTE_SETTINGS_DESIGN.md)(F22).
