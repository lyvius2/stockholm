...
- **변경 이력은 본문이 아니라 문서 끝 `## Changes` 표에 쓴다.** 본문에 `[확정 YYYY-MM-DD]` 같은 날짜 표기를 넣지 않는다. 본문은 현재 상태만 말하고, 무엇이 언제 바뀌었는지는 표 한 줄로 남긴다. HANDOFF.md(인수인계 일지)는 예외다. 이전에 본문에 적힌 날짜 표기는 그 문서를 손볼 때 걷어 낸다.
