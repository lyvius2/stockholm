...
/**
 * 금액. 통화가 다른 값끼리는 연산하지 않음(`CurrencyMismatchException`). 생성자는 통화 자릿수에 맞는 금액만 받고, 반올림은 팩토리 `of`가 함.
 */
data class Money(val amount: BigDecimal, val currency: Currency) : Comparable<Money> {
    init {
        if (amount.scale() != currency.scale) {
            throw InvalidValueException("$currency 금액은 소수 ${currency.scale}자리여야 함: $amount")
        }
    }
    ...
    fun ratioTo(base: Money): Percent {
        if (sameCurrency(base).isZero) throw InvalidValueException("0 금액에 대한 비율은 정의되지 않음")
    ...
    fun convert(rate: ExchangeRate): Money {
        if (rate.from != currency)
            throw CurrencyMismatchException("환율 ${rate.from}→${rate.to} 은 $currency 금액에 쓸 수 없음")
    ...
    private fun sameCurrency(other: Money): Money {
        if (other.currency != currency)
            throw CurrencyMismatchException("$currency 와 ${other.currency} 는 함께 계산할 수 없음")
        return other
    }
